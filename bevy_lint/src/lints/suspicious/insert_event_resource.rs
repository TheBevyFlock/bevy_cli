...
declare_bevy_lint! {
    ...
    /// # Motivation
    ///
    /// Unless you have intentionally and knowingly initialized the `Events<T>` resource in this
    /// way, events and their resources should be initialized with `App::add_event()` because it
    /// automatically handles dropping old events. Just adding `Events<T>` makes no such guarantee,
    /// and will likely result in a memory leak.
    ///
    /// For more information, please see the documentation on [`App::add_event()`] and
    /// [`Events<T>`].
    ///
