...
declare_bevy_lint! {
    ...
    /// # Motivation
    ///
    /// Reflection lets programs inspect type information at runtime. It is commonly used by tools
    /// to view and edit ECS information while the program is running. Reflection is opt-in,
    /// however, and easy to forget since you need to `#[derive(Reflect)]` for each type that uses
    /// it.
    ///
    /// # Known issues
    ///
    /// This lint will suggest `#[derive(Reflect)]` even if it cannot be applied. (E.g. if one of
    /// the fields does not implement `Reflect`.) For more information, please see [#141].
    ///
