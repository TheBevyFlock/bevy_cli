...
declare_bevy_lint! {
    ...
    /// # Motivation
    ///
    /// Modifying the camera in `FixedUpdate` can cause jittery, inconsistent, or laggy visuals
    /// because `FixedUpdate` may not run every render frame, especially on games with a high FPS.
    ///
    /// # Known Issues
    ///
    ...
    /// ```
    ///
    /// Any system that modifies the camera in a user-visible way should be run every render frame.
    /// The `Update` schedule is a good choice for this, but it notably runs _after_ `FixedUpdate`.
    /// You can use the `RunFixedMainLoop` schedule with the
    /// `RunFixedMainLoopSystem::BeforeFixedMainLoop` system set to run a system before
    /// `FixedUpdate`:
    ///
    /// ```
    /// # use bevy::prelude::*;
    ...
    ///         // `FixedUpdate`. This will still run every render frame, though, so there won't be
    ///         // any lag!
    ///         .add_systems(
    ///             RunFixedMainLoop,
    ...
    /// ```
    ///
    /// For more information, check out the [physics in fixed timestep
