...
declare_bevy_lint! {
    ...
    /// # Motivation
    ///
    /// Zero-sized types (ZSTs) are types that have no size because they contain no runtime data.
    /// Any information they may hold is known at compile-time in the form of [constant generics],
    /// which do not need to be queried. As such, ZSTs are better used as query filters instead of
    /// query data.
    ///
    ...
    /// # Known Issues
    ///
    /// This lint raises false positives on queries like `Has<T>` and `AnyOf<T>` because they are
    /// ZSTs, even though they still retrieve data from the ECS. Please see [#279] for more
    /// information.
    ///
