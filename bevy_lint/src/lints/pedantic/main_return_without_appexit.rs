...
declare_bevy_lint! {
    ...
    ///
    /// This lint will not be emitted if `fn main()` returns a non-[`unit`] type, even if that type
    /// is not `AppExit`.
    ///
    /// # Motivation
    ///
    /// `AppExit` is used to determine whether the `App` exited successfully or due to an error.
    /// Returning it from `main()` will set the [exit code], which can be read by external
    /// processes. Not returning any `AppExit` may cause external processes to believe the program
    /// gracefully exited, when in reality it may have crashed.
    ///
