...
//! implementations, except we implement for `&'static T` instead of plain `T`.
...
impl Resource for &'static MyResource {}

struct MyEvent;
//~^ ERROR: defined an event without a `Reflect` implementation
//~| HELP: `Reflect` can be automatically derived

impl Component for &'static MyEvent {
    const STORAGE_TYPE: StorageType = StorageType::Table;
}

//~v NOTE: `Event` implemented here
impl Event for &'static MyEvent {}
